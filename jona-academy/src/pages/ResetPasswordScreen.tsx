  ...
  if (!token) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, marginBottom: 10, color: 'var(--text-primary)' }}>Link i pavlefshëm</h2>
        <p style={{ color: 'var(--text-secondary)', marginBottom: 24, fontSize: 14 }}>Ky link për rivendosjen e fjalëkalimit mungon ose është i pasaktë.</p>
        <button className="btn btn-primary btn-full" onClick={() => navigate('/forgot-password')} style={{ padding: '16px' }}>Kërko Link të Ri</button>
      </div>
    )
  }

  const ndrysho = async () => {
    if (fjalekalimRi.length < 6) { setError('Fjalëkalimi duhet të ketë të paktën 6 karaktere.'); return }
    if (fjalekalimRi !== konfirmo) { setError('Fjalëkalimet nuk përputhen.'); return }
    setLoading(true)
    setError('')
    try {
      await resetPassword(token, fjalekalimRi)
      setSuccess(true)
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ background: 'var(--bg-primary)', borderBottom: '1px solid var(--border)', padding: '16px 20px' }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: 'var(--text-primary)' }}>Vendos Fjalëkalim të Ri</h2>
      </div>
      ...
      <div style={{ flex: 1, padding: '28px 20px' }}>
        ...
        {success && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(74,155,111,0.1)', border: '1px solid rgba(74,155,111,0.3)', borderRadius: 8, padding: '10px 14px', marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 13, color: '#4A9B6F', fontWeight: 600, margin: 0 }}>Fjalëkalimi u ndryshua! Duke të kthyer te hyrja...</p>
          </div>
        )}
