...
import {
  UserIcon, LockIcon, BellIcon, BookOpenIcon,
...
export default function SettingsScreen() {
  ...
  const [njoftime, setNjoftime] = useState(true)
  ...
  const Rreshti = ({ Icon, label, value, onPress, isToggle, toggleValue, onToggle, last }: {
    Icon: React.FC<{ size?: number; color?: string; strokeWidth?: number }>
    label: string; value?: string; onPress?: () => void
    isToggle?: boolean; toggleValue?: boolean; onToggle?: () => void; last?: boolean
  }) => (
    <button
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)', paddingBottom: 40 }}>
      ...
      <div style={{ paddingTop: 16 }}>
        ...
        <Seksioni titulli="Llogaria">
          <Rreshti Icon={UserIcon}       label="Ndrysho Profilin"     onPress={() => {}} />
          <Rreshti Icon={LockIcon}       label="Ndrysho Fjalëkalimin" onPress={() => {}} />
          <Rreshti Icon={BookOpenIcon}   label="Email" value="sm@teenchallengealbania.org" last />
        </Seksioni>

        <Seksioni titulli="Njoftimet">
          <Rreshti Icon={BellIcon}       label="Njoftime Push"        isToggle toggleValue={njoftime}      onToggle={() => setNjoftime(n => !n)} />
          <Rreshti Icon={BookOpenIcon}   label="Azhurnime me Email"   isToggle toggleValue={emailUpdates}  onToggle={() => setEmailUpdates(n => !n)} last />
        </Seksioni>
